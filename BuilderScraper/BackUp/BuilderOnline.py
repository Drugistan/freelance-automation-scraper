...
#           count = count + 1
#           counts = str(count)
#           links_list.append(x.find_element(By.CLASS_NAME, 'headline2').get_attribute('href'))
#           titles_list.append(x.find_element(By.CLASS_NAME, "headline2").text)
#           image_list.append(x.find_element(By.CLASS_NAME,
#                                            f"//div[@class='results -standard_layout']/div[@class='result type-article split'][{counts}]/div[@class='thumbnail-wrapper']/a[@class='result-image-wrapper']/img").get_attribute(
#               'src'))
#
#       data = {
#           "Article Title": titles_list,
#           "Article Image": image_list,
#           "Article Links": links_list
#       }
#       df = pd.DataFrame(data)
#       df.to_csv("builderOnline.csv", index=False, mode='a', header=False)
